...
(define (parse sexp)
  (match sexp
    ...
    [(list 'rename (list source 'as dest) 'in body) (rename-as source
                                                   dest
                                                   (parse body))]
    [(list 'begin exp exprs) (begin-exp (cons (parse exp) (parse exprs)))]
...
;; begin tests
;(test (parse '{with {x 5} {begin {with {x 1} x} x}}) (with 'x (num 5) (begin-exp (cons (with 'x (num 1) (id 'x)) (id 'x)))))

;(test (parse '{begin {+ 5 6} {{* 2 3} {- 9 1}}}) (begin-exp (cons (binop + (num 5) (num 6))
;                                                                      (cons (binop * (num 2) (num 3))
;                                                                            (binop - (num 9) (num 1))))))
;
...
(define (subst target-exp target-id val)
  (type-case ReWAE target-exp
    ...
    [if0 (scrutinee t e) (if0 (subst scrutinee target-id val)
     ...
     [with (bound-id named-expr bound-body)
          (if (symbol=? bound-id target-id)
              ;; little confused here, when id's are = 
              (with bound-id
    ...
    [lazy-with (bound-id named-expr bound-body) 0 ]
    [rename-as (source-id dest-id body) 0]
    [begin-exp (exprs) 0 ]
  ...
  (type-case ReWAE an-ae
    ...
    [lazy-with (bound-id named-expr bound-body)
               (interp (subst bound-body
                       ...
                       (num (interp named-expr))))]
    [rename-as (source dest body) (subst (interp body)
                                         (interp source)
                                         (interp body))] ; (subst target-exp target-id val) ;; rename x as y in (+ x 3)
    [begin-exp (exprs) 0]
    [id (v) (error "")] 
    ))
